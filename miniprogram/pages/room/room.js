...
Page({
  ...
  onLoad: function(options) {
    ...
    this.setData({
      come: app.globalData.userInfo.nickName.toLowerCase(),
      to: options.chatTo || '',
      chatWrapperMaxHeight,
      chatTo: options.chatTo || '',
      loginAccountLogo: app.globalData.userInfo.avatarUrl
    })
    this.getHistory()
    let list = wx.getStorageSync(`RawMessageList_Add_Msg_${this.data.chatTo}`) || [];
    this.setData({
      messageArr: this.convertRawMessageListToRenderMessageArr(list) || [],
    })
    // 重新计算所有时间
  ...
  saveChatMessageListToStore(rawMsg, handledMsg) {
    let list = wx.getStorageSync(`RawMessageList_Add_Msg_${this.data.chatTo}`) || [];
    list.push(rawMsg)
    wx.setStorageSync(`RawMessageList_Add_Msg_${this.data.chatTo}`, list)
    this.setData({
      messageArr: this.convertRawMessageListToRenderMessageArr(list) || [],
    })
  },
